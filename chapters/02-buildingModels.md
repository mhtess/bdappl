...
As scientists, we very often in the situation of having observed some data in our *sample* and trying to say something about the underlying, generating probability.
...
First, we explicitly represent out state of knowledge about `probabilityOfHelping` *a priori*.
...
We can combine this prior distribution over the parameter with the generative process of the data, which we wrote down above.
...
This is process is called *Bayesian inference* and represents the mathematically correct way of inductive reasoning about the underlying probability that generated our data.
...
If we repeat this function many times, we will estimate the proportion of `probabilityOfHelping`s under our prior (i.e., between 0 - 1) that give rise to our observed data.
...
In principle, any value `probabilityOfHelping` **could** give rise to our data, but intuitively some values are more likely to than others (e.g., a `probabilityOfHelping` = 0.2, would produce 15 out of 20 successes with probability proportional to $$0.2^{15} + 0.8^5$$, which is not as likely as a `probabilityOfHelping` = 0.8 would have in producing 15 out of 20 success).
